...
class TestBuild(TestCase):
    ...
    def test_builds_without_dist_dir(self) -> None:
        with patch("dev.tasks.build.os.path.isdir", return_value=False), patch(
            "dev.tasks.build.run_process"
        ) as mock_run:
    ...
    def test_removes_dist_dir_before_building(self) -> None:
        with patch("dev.tasks.build.os.path.isdir", return_value=True), patch(
            "dev.tasks.build.shutil.rmtree"
        ) as mock_rmtree, patch("dev.tasks.build.run_process"):
            rc = BuildTask.execute()
