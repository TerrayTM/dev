...
class TestChain(TestCase):
    ...
    def test_returns_ok_when_all_subtasks_pass(self) -> None:
        with (
            patch.object(LintTask, "execute", return_value=ReturnCode.OK),
            patch.object(UnusedTask, "execute", return_value=ReturnCode.OK),
            patch.object(DocTask, "execute", return_value=ReturnCode.OK),
            patch.object(SpellTask, "execute", return_value=ReturnCode.OK),
        ):
    ...
    def test_returns_highest_return_code(self) -> None:
        with (
            patch.object(LintTask, "execute", return_value=ReturnCode.OK),
            patch.object(UnusedTask, "execute", return_value=ReturnCode.FAILED),
            patch.object(DocTask, "execute", return_value=ReturnCode.OK),
            patch.object(SpellTask, "execute", return_value=ReturnCode.OK),
        ):
            rc = ChainTask.execute()
    ...
    def test_runs_all_subtasks(self) -> None:
        with (
            patch.object(LintTask, "execute", return_value=ReturnCode.OK) as mock_lint,
            patch.object(
                UnusedTask, "execute", return_value=ReturnCode.OK
            ) as mock_unused,
            patch.object(DocTask, "execute", return_value=ReturnCode.OK) as mock_doc,
            patch.object(
